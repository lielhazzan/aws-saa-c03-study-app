...
try {
  ...
  questionsList.forEach(q => {
    // Only process single select
    if (q.questionType === 'multi_select') return;
    
    const correctOptions = q.options.filter(o => o.isCorrect);
    if (correctOptions.length !== 1) return; // Must have exactly one correct answer
  ...
  console.log(`Successfully added ${addedCount} questions from the JSON file.`);
